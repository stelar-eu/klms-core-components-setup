...
def resync_user_permissions(user: dict, token):
    """
    Resyncs the user permissions for the given user representation.
    """
    if not user:
        return
    ...
    try:
        ...
        sub, lusername, lemail, additional_info = user.get("oauth_id"), user.get("username"), user.get("email"), {"groups": user.get("groups")}
        ...
        return model.user.get_user_by_id(login_result.user_obj)
...
def validate_sso_oauth_token(token):
    ...
    try:
        ...
        sub, lusername, lemail, additional_info = get_sub_username_email_from_token(
            decoded_id_token, None, service.config, False, True
        )
