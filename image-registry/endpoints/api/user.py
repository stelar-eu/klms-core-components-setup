...
from auth.oauth import resync_user_permissions
from auth.auth_context import get_authenticated_user, get_sso_token
...
class User(ApiResource):
    ...
    schemas = {
        ...
        "ResyncUser": {
            "type": "object",
            "description": "Fields required to resync SSO permissions.",
            "required": [
                "oauth_id",
                "username",
                "email",
    ...
    @nickname("resyncSSOPermissions")
    @require_scope(scopes.SUPERUSER)
    @require_scope(scopes.ORG_ADMIN)
    @validate_json_request("ResyncUser")
    def patch(self):
        actor = get_authenticated_user()

        if SuperUserPermission().can():
            if actor is None or actor.organization or not UserReadPermission(actor.username).can():
                raise InvalidToken("Requires authentication", payload={"session_required": False})
        
            user_data = request.get_json()
            result = resync_user_permissions(user_data, get_sso_token())
            try:
                return user_view(result)
            except Exception as e:
                logger.info("Exception occurred while resyncing user permissions", exc_info=e)
                raise request_error(message="Exception occurred while resyncing user permissions", exception=e)
        else:
            raise InvalidToken("Requires super user authentication", payload={"session_required": False})
